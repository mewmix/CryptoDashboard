...
headers = {
    'authority': 'eos.dfuse.eosnation.io',
...
response = requests.get('https://eos.dfuse.eosnation.io/v0/state/table', params=params, headers=headers)
...
con_weight = str(connector_weight).replace("EOS" , "")
base_bal = str(base_balance).replace("RAM" , "")
quote_bal = str(quote_balance).replace("EOS" , "")
